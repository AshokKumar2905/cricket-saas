...
import { BookOpen, Clock, Activity, Server, Sparkles, ChevronRight } from "lucide-react";
    ...
    <div className="w-full relative overflow-x-hidden min-h-screen flex flex-col flex-1 bg-zinc-950 text-zinc-100 font-sans selection:bg-emerald-500 selection:text-zinc-950">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 w-full relative z-10 flex-1 flex flex-col bg-transparent">
        ...
        <div className="mb-12 border-l-4 border-emerald-500 pl-4">
          ...
          <p className="text-xs sm:text-sm text-zinc-400 mt-2 max-w-xl leading-relaxed font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 bg-transparent">
            {blogs.map((article, idx) => (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className="bg-gradient-to-br from-zinc-900/80 to-zinc-950/90 border border-zinc-800/80 rounded-3xl overflow-hidden flex flex-col justify-between shadow-2xl group backdrop-blur-md transition-all duration-300"
              >
                <div>
                  ...
                  <div className="w-full aspect-[16/10] relative overflow-hidden bg-zinc-950 border-b border-zinc-800/60">
                    <img 
                      ...
                      alt={article.title} 
                      className="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500 group-hover:opacity-90"
                      onError={(e) => {
                        // 🚀 Fallback image handler maps verified mock asset if backend data stream breaks
                    ...
                    <span className="absolute top-4 left-4 text-[9px] font-mono font-bold uppercase bg-zinc-900/90 border border-zinc-700 rounded-lg text-emerald-400 px-2.5 py-1 shadow-md tracking-wider">
                      {article.category}
                    </span>
                  </div>
                  ...
                  <div className="p-6 space-y-3">
                    <div className="flex gap-4 items-center text-[10px] font-mono text-zinc-500 uppercase tracking-wider font-bold">
                      <span className="flex items-center gap-1.5">
                        <Clock className="w-3.5 h-3.5 text-zinc-500" /> {article.read_time || "6 Min Read"}
                      </span>
                    </div>
                    <h2 className="text-md font-black uppercase text-white tracking-tight group-hover:text-emerald-400 transition-colors leading-snug">
                      {article.title}
                    </h2>
                ...
                <div className="p-6 pt-0">
                  <div className="pt-4 border-t border-zinc-800 flex items-center justify-between">
                    <Link 
                      ...
                      className="w-full py-2.5 px-4 bg-zinc-950 border border-zinc-800 hover:border-emerald-500/30 rounded-xl font-mono font-bold uppercase tracking-wider text-[10px] text-zinc-400 hover:text-emerald-400 flex items-center justify-center gap-1.5 transition-all active:scale-[0.98] shadow-md cursor-pointer"
                    >
                      Analyze Protocol Report <ChevronRight className="w-3.5 h-3.5" />
                    </Link>
                    <Sparkles className="w-3.5 h-3.5 text-amber-400/0 group-hover:opacity-100 group-hover:text-amber-400/80 transition-all duration-300 transform group-hover:rotate-12 pointer-events-none hidden" />
                  </div>
                </div>
